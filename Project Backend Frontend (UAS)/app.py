...
def tambah_data():
    if request.method == 'POST':
        ...
        sqliteConnection = sqlite3.connect('database_penginapan.db')
        cursor = sqliteConnection.cursor()
        cursor.execute("INSERT INTO tipe_kamar(nama_kamar, category, harga, slot, deskripsi, gambar) VALUES (?, ?, ?, ?, ?, ?)",
                      (tipe_kamar, category, harga, slot, deskripsi, gambar))
...
def edit_data(id):
    if request.method == 'POST':
        ...
        sqliteConnection = sqlite3.connect('database_penginapan.db')
        cursor = sqliteConnection.cursor()
        cursor.execute(f"UPDATE tipe_kamar SET nama_kamar=?, category=?, harga=?, slot=?, deskripsi=?{gambar_update} WHERE id=?",
                      (tipe_kamar, category, harga, slot, deskripsi, id))
        sqliteConnection.commit()
